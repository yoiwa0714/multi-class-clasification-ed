...
class EDGenuine:
    ...
    # C言語定数の現代化 - 1999年MAX=1000制約除去、完全動的対応
    MAX_OUTPUT_NEURONS = 10       # 最大出力ニューロン数（原典準拠）
    # MAX_UNITSは動的設定（__init__内でデータサイズに応じて計算）
    
    @classmethod
    def calculate_required_units(cls, train_size, test_size):
        """
        必要ユニット数の動的計算（現代コンピュータ対応）
        
        1999年のMAX=1000制約を除去し、データサイズに応じた
        完全動的メモリ管理を実装
    ...
    def __init__(self, hyperparams=None):
        ...
        # 完全動的ユニット数計算（現代コンピュータ対応）
        train_size = getattr(hyperparams, 'train_samples', 1000)
        test_size = getattr(hyperparams, 'test_samples', 1000)
        
        required_units, memory_estimate = self.calculate_required_units(train_size, test_size)
        self.max_units = required_units  # 動的MAX_UNITS設定
        
        if self.hyperparams.verbose:
            print(f"🚀 現代化対応: 動的ユニット数 = {self.max_units} (推定メモリ: {memory_estimate:.1f}GB)")
            print(f"   1999年MAX=1000制約除去済み、現代コンピュータ最適化")
        ...
        self.error = 0.0
        self.error_count = 0
        self.pattern_types = [0] * (self.max_units+1)  # 各出力ニューロンのパターンタイプ（動的サイズ対応）
    ...
    def load_external_data(self, input_data: np.ndarray, class_labels: np.ndarray):
        ...
        max_label = int(np.max(class_labels))
        min_label = int(np.min(class_labels))
        
        # Fashion-MNISTとMNISTの場合は確実に10クラスに設定
        if input_size == 784 and min_label == 0 and max_label == 9:
            print(f"🔍 MNIST/Fashion-MNIST データセット検出: 10クラスに設定")
            num_classes = 10
        else:
            # ラベルが0から始まる連続した整数でない場合もあるので、より安全な計算
            expected_classes = max_label + 1  
            if expected_classes != num_classes:
                print(f"⚠️ 警告: ユニークラベル数({num_classes})と期待されるクラス数({expected_classes})が一致しません")
                print(f"   ユニークラベル: {unique_labels}")
                print(f"   最大ラベル: {max_label}")
                # より安全な方を選択
                num_classes = max(num_classes, expected_classes)

        self.num_patterns = patterns

        print(f"外部データ読み込み: {patterns}パターン, {input_size}次元, {num_classes}クラス")

        # 動的配列サイズ拡張（ed_multi.prompt.md準拠）
        if patterns > self.MAX_UNITS:
            print(f"🔧 配列サイズ拡張: {self.MAX_UNITS} → {patterns}")
            # 入力・教師データ配列を動的に拡張
            self.input_data = np.zeros((patterns + 1, self.MAX_UNITS + 1), dtype=np.float64)
            self.teacher_data = np.zeros((patterns + 1, num_classes + 1), dtype=np.float64)
        ...
        for p in range(patterns):
            ...
            if true_class < num_classes and true_class >= 0:  # 安全チェック
                self.teacher_data[p][true_class] = 1.0
            else:
                print(f"❌ エラー: ラベル{true_class}がクラス数{num_classes}を超えています")
        ...
        if input_size == 784:  # MNIST画像
            ...
            # hidden_neuronsの代わりにhidden_layersの最初の値を使用
            hidden_neurons = self.hyperparams.hidden_layers[0] if hasattr(self.hyperparams, 'hidden_layers') and self.hyperparams.hidden_layers else 128
    ...
    def load_dataset(self, train_size=1000, test_size=1000, use_fashion_mnist=False, total_epochs=1):
        ...
        # 🔧 【最適化】メモリ効率的データロード：エポック時のオンデマンド生成
        # エポック毎独立データは実行時生成、事前大量読み込みを回避
        actual_train_size = train_size  # エポック単位サイズ（実行時に独立生成）
        actual_test_size = test_size   # テスト：全エポック共通（標準手法）
        
        dataset_name = "Fashion-MNIST" if use_fashion_mnist else "MNIST"
        if self.hyperparams.verbose:
            print(f"{dataset_name}データセット読み込み: 訓練{train_size}/エポック×{total_epochs}エポック、テスト{actual_test_size}個（共通）")
            print(f"🎯 メモリ最適化方式: エポック実行時に{train_size}個の独立訓練データを動的生成")
        ...
        # 🚀 【最適化】訓練データ準備：動的生成用の基盤情報のみ格納
        # エポック実行時に独立データを生成するため、ここでは軽量準備のみ
        
        # 全訓練データインデックス取得（動的サンプリング用）
        self.all_train_indices = list(range(len(train_dataset)))
        self.train_dataset_ref = train_dataset  # 実行時アクセス用参照
        
        # 動的生成情報の保存（実行時使用）
        self.dynamic_train_size = actual_train_size
        self.total_epochs_info = total_epochs
        
        # 軽量な初期データ準備（後方互換性確保）
        # エポック実行時に動的生成するため、ここでは空のリストのみ
        train_inputs = np.array([])  # 後方互換性のため空配列として初期化
        ...
        if self.hyperparams.verbose:
            print(f"✅ {dataset_name}データ準備完了（メモリ最適化方式）:")
    ...
    def generate_epoch_data(self, epoch: int) -> tuple:
        """
        エポック毎の独立訓練データを動的生成（メモリ最適化版）
        ed_multi.prompt.md準拠：エポック毎に完全独立したデータセットを使用
        
        Args:
            epoch: エポック番号（シード計算用）
        Returns:
            tuple: (train_inputs, train_labels) - エポック専用の独立データ
        """
        if not hasattr(self, 'train_dataset_ref'):
            raise RuntimeError("動的データ生成が初期化されていません")
        
        # エポック毎に異なるシードで完全独立サンプリング
        import random
        epoch_seed = 42 + epoch * 1000  # エポック毎に異なるシード
        random.seed(epoch_seed)
        np.random.seed(epoch_seed)
        
        # エポック専用の独立データサンプリング
        if len(self.all_train_indices) >= self.dynamic_train_size:
            # 十分なデータがある場合：重複なしランダムサンプリング
            selected_indices = random.sample(self.all_train_indices, self.dynamic_train_size)
        else:
            # データ不足の場合：重複サンプリング
            selected_indices = [random.choice(self.all_train_indices) for _ in range(self.dynamic_train_size)]
        ...
        for idx in selected_indices:
            image, label = self.train_dataset_ref[idx]
            flattened = image.flatten().numpy()
        ...
        if self.hyperparams.verbose and epoch % 10 == 0:
            print(f"🔄 エポック{epoch}: 独立データ生成完了 - {len(train_inputs)}サンプル")
    ...
    def neuro_teach_calc(self, indata_tch: List[float]):
        ...
        # デバッグ出力（詳細モード時のみ）
        if hasattr(self, 'hyperparams') and self.hyperparams.verbose and len(amine_debug_info) > 0:
            print(f"🔍 アミン濃度設定: 総誤差={total_error:.3f}, 詳細={amine_debug_info[:3]}")  # 最初の3つだけ表示
    ...
    def _neuro_weight_calc_vectorized(self):
        ...
        for n in range(self.output_units):
            ...
            # デバッグ情報収集
            total_weight_updates += len(weight_update)
            current_max = np.max(np.abs(weight_update)) if len(weight_update) > 0 else 0
            if current_max > max_weight_update:
                max_weight_update = current_max
        
        # デバッグ出力（詳細モード時のみ）
        if hasattr(self, 'hyperparams') and self.hyperparams.verbose:
            print(f"🔍 重み更新: 総更新数={total_weight_updates}, 最大更新幅={max_weight_update:.6f}")
    ...
    def train_epoch_with_buffer(self, results_buffer, epoch: int,
        ...
        for pattern in pattern_iterator:
            ...
            prediction_outputs = self.get_output_values(indata_input)  # 出力値配列取得
            predicted_class = self.predict(indata_input)  # 予測クラス取得
            correct = (predicted_class == true_class)
            # ED法準拠の誤差計算: テスト時と同じ計算方式を使用
            teacher_onehot = self._onehot_encode(true_class, self.output_units)
            error = sum(abs(teacher_onehot[j] - prediction_outputs[j]) for j in range(self.output_units)) / self.output_units
        ...
        test_prediction_outputs = self.get_output_values_batch(test_inputs)  # 出力値配列取得
        test_predictions = self.predict_batch(test_inputs)  # 予測クラス取得
        ...
        for i, (prediction_class, prediction_outputs, test_label) in enumerate(zip(test_predictions, test_prediction_outputs, test_labels)):
    ...
    def train_epoch_with_minibatch(self, results_buffer, epoch: int,
                                   train_inputs: np.ndarray, train_labels: np.ndarray,
                                   test_inputs: np.ndarray, test_labels: np.ndarray,
        ...
        # 動的生成データの場合、original_indicesは動的に生成
        if hasattr(self, 'train_original_indices') and len(self.train_original_indices) > 0:
            train_original_indices = self.train_original_indices
        else:
            # 動的生成の場合、仮想インデックスを生成
            train_original_indices = np.arange(len(train_inputs))
    ...
    def predict(self, input_pattern: List[float]) -> int:
        ...
        for n in range(self.output_units):
            output_value = self.output_outputs[n][self.input_units + 2]
    ...
    def get_output_values(self, input_pattern: List[float]) -> List[float]:
        ...
        output_values = []
        for n in range(self.output_units):
            output_values.append(self.output_outputs[n][self.input_units + 2])
    ...
    def get_output_values_batch(self, input_patterns: np.ndarray) -> np.ndarray:
        """
        バッチ出力値配列取得（ED法準拠の誤差計算用）
        各パターンの全出力値を配列として返す
        """
        output_arrays = []
        for pattern in input_patterns:
            output_values = self.get_output_values(pattern.tolist())
            output_arrays.append(output_values)
