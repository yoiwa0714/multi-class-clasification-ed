...
class HeatmapRealtimeVisualizer:
    ...
    def setup_visualization(self, layer_activations: List[np.ndarray]):
        ...
        try:
            ...
            # 上部: タイトル・パラメータエリア (高さ比3)
            # 下部: ヒートマップエリア (高さ比10)  
            gs_main = gridspec.GridSpec(2, 1, figure=self.fig, height_ratios=[4, 10], hspace=0.15)
            ...
            # 上部右: パラメータ表示エリア（LIF・ED上下配置）
            if self.show_parameters:
                gs_params = gridspec.GridSpecFromSubplotSpec(
                    2, 1, gs_upper[0, 1], hspace=0.3
                )
                self.param_ax_lif = self.fig.add_subplot(gs_params[0, 0])
                self.param_ax_ed = self.fig.add_subplot(gs_params[1, 0])
    ...
    def update_learning_results(self, results_data: Dict):
        ...
        # サブタイトルを即座に更新
        if self.is_initialized and self.fig:
    ...
    def _update_subtitle(self):
        ...
        if hasattr(self, 'title_ax') and self.title_ax:
            ...
            # メインタイトル
            self.title_ax.text(0.5, 0.7, "ED-SNN ヒートマップリアルタイム表示", 
    ...
    def update_display(self, layer_activations: List[np.ndarray], 
        ...
        if not self.is_initialized:
            self.initialize_display(layer_activations)
        ...
        # 表示タイミング制御（フェーズ2で活用）
        if not self.timing_controller.should_update():
            return  # 指定間隔に達していない場合はスキップ
            
        try:
            # 1. ウィンドウ存在確認
            if self.fig is None or not plt.fignum_exists(self.fig.number):
                print("🎯 ヒートマップウィンドウが閉じられました")
                return
        ...
        try:
            ...
            # 2. メインタイトル更新
            if self.title_ax is not None:
                self.title_ax.clear()
                self.title_ax.axis('off')
                
                # メインタイトル（上部）
                main_title = "ED-SNN ヒートマップリアルタイム表示"
                self.title_ax.text(0.05, 0.8, main_title, transform=self.title_ax.transAxes,
                                  fontsize=18, weight='bold', verticalalignment='center')
                
                # サブタイトル（下部）- フォント高さの半分程度の空白を空けて配置
                # 新仕様: エポック#: A    正解クラス: B    予測クラス: C
                sub_title = f"エポック#: {epoch}    正解クラス: {true_label if true_label >= 0 else '-'}    予測クラス: {predicted_label if predicted_label >= 0 else '-'}"
                
                self.title_ax.text(0.05, 0.3, sub_title, transform=self.title_ax.transAxes,
                                  fontsize=16, weight='bold', verticalalignment='center')
