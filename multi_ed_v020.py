#!/usr/bin/env python3
"""
純正ED法（Error Diffusion Learning Algorithm）Python実装 v0.2.0 - 公開準備完成版
Original C implementation by Isamu Kaneko (1999) - Production Ready Release

金子勇氏のオリジナルC実装を完全に忠実に再現 + 公開向け最適化完成

【ed_genuine.prompt.md完全準拠確認済み - 2025年9月7日】
...
python multi_ed_v020.py --lr 0.9 --epochs 5 --train 200 --test 50 --viz --v

# Fashion-MNIST（高性能設定）
python multi_ed_v020.py --fashion --lr 0.9 --epochs 5 --train 200 --test 50 --viz --v

# 図表保存付きCPU実行
python multi_ed_v020.py --fashion --cpu --amine 1.0 --diffusion 0.8 --hidden 128 --save_fig results

# 詳細プロファイリング実行
python multi_ed_v020.py --epochs 10 --viz --profile --save_fig benchmark
