...
[GitHub ED-ANN](https://github.com/yoiwa0714/ed-ann)
...
- リアルタイム可視化グラフウィンドウのサイズは表示中に変更することができます。通常のウィンドウサイズの変更操作と同様に、ウィンドウの四隅をマウスで操作してください。
...
- **学習時間**: 大幅な学習時間増加の防止
  - 一見するとクラス数分の重み配列を設けたことによって学習時間が10倍近くになりそうに思えますが、実際は一つの学習データにつきその学習データの正解ラベル(正解クラス)のみを学習する仕様にしているため、学習時間が大幅に増加することはありません。(ただし、通常のネットワークの学習時間と比較/検証できた訳ではないため、推測による部分が大きいです。)
...
verifyオプションを使用して学習結果のデータをCSVファイルに書き出し、そのCSVファイルのデータを確認することで検証を行っています。

## 更新履歴

### v1.0.0 (2025-08-23) - 初公開版
- ED法によるマルチクラス分類の基本実装
- PyTorch統合による高効率学習
- リアルタイム可視化機能
- MNIST手書き数字認識による実証実験
...
本プロジェクトは Apache License 2.0 ライセンスの下で公開されています。
